...
Development harness plus a versioned evaluation policy corpus under `data/raw/`, versioned evaluation questions under `evals/datasets/`, a dataset validator, seller-scoped document parsing, configurable fixed/recursive chunking, deterministic embeddings, SQLite chunk indexing, seller-scoped dense top-K retrieval, a reproducible retrieval baseline, and evidence-grounded local Ollama generation with trusted citations. Generation-quality and later evaluation runners are not implemented.
...
Requires Python 3.10+ and a POSIX shell. `./init.sh` creates the git-ignored `.venv`, installs the application dependency set from `requirements.txt`, and runs the harness with `.venv/bin/python`. The application uses only the Python standard library. Phase 3 generation calls a separately installed local Ollama service as specified by ADR 0003; implementation checks inject a fake transport and do not certify model quality. Git is used for inspecting changes. Node.js is only needed to rerun the external harness-creator skill validator.
...
`.venv/bin/python scripts/harness.py verify dataset` validates the complete Phase 1 corpus and annotations. `verify unit`, `verify integration`, and `verify isolation` run ingestion-through-generation implementation checks. `verify retrieval` reproduces the pinned Recall@5 and MRR@10 baseline in `evals/reports/retrieval-baseline-v1.json`. `verify all` still exits 3 while generation, abstention, regression, and system evaluation checks are unavailable; this is not a passing application build.
