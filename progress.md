...
## Follow-up — 2026-09-18: approved Phase 3 grading, blocked on runtime

- The user approved the proposed Phase 3 hybrid grader and thresholds. Added ADR `docs/decisions/0004-phase-3-generation-evaluation.md`, updated `docs/EVALUATION.md` and task acceptance criteria, cleared the decision blocker and opened only `p3-generation-evaluation`.
- Installed local Ollama judge `qwen2.5:7b-instruct-q4_K_M` (resolved 4.7 GB Q4_K_M artifact) alongside the existing 986 MB `qwen2.5:1.5b-instruct` generator. No Python package, cloud credential or provider SDK was added.
- Added the stdlib `scripts/evaluate_generation.py` implementation for 31 answerable cases in gold and retrieved views, 20 calibration examples, hybrid deterministic/semantic grading, runtime/model/token/latency reporting, threshold enforcement, baseline comparison and temporary run reuse across generation/regression/system gates. Wired the three gates and added four generation, one regression, one system and harness availability checks.
- First implementation check failed because two proposed calibration IDs did not exist; corrected them to the actual answerable cases `aurora-adversarial-03` and `beacon-simple-16`. The next real run exposed duplicate `[0, 0]` point IDs from the judge despite `uniqueItems`; the parser now validates IDs then deduplicates them without changing scores or thresholds, with a regression check.
- Post-fix non-model checks passed: generation 4/4, regression 1/1, system 1/1, harness 54/54, `.venv/bin/python scripts/harness.py check`, and `git diff --check`.
- The third full real-service attempt ran beyond 900 seconds and was stopped because the harness generation timeout is 900 seconds. It had passed the prior duplicate failure and was still processing judge calls, but produced no complete report or baseline. No result was fabricated, no threshold lowered, no case removed and no timeout changed.
- Marked `p3-generation-evaluation` blocked after the required three-attempt stop. Human review must choose faster/hardware-accelerated Ollama, approve a measured timeout or workload change, or select a smaller independent local judge while preserving the approved quality thresholds. Phase 3 remains in progress/unverified; Phase 4 was not started.

## Follow-up — 2026-09-18: 1800-second baseline attempts and quality blocker

- Resumed only `p3-generation-evaluation` after the user approved 1800-second timeouts for generation, regression and system. Updated the three harness timeouts; generation/regression/system tests and the 54-test harness suite passed before the real run.
- Ran three complete real-service baseline attempts without lowering thresholds or removing cases. Attempt 1 completed in about 17 minutes but had 11 structured-output/judge failures and calibration kappa `0.7358490566037734`. Attempt 2 removed execution failures (62/62 outputs) but calibration kappa remained `0.7931034482758621`. The final bounded evaluator maps unsupported judgments only to actual answer sentences; its 20-example calibration reached agreement `1.0` and kappa `1.0`.
- Final run `generation-baseline-a3db74bd764a285d` executed 62/62 outputs with 0 errors, 0 skips, 0 seller leaks and local cost `$0`. Gold: 18/31 faithful, 31/31 relevant, 18/31 citation-correct, 48/48 conditionally complete, 13 unsupported sentences. Retrieved: 18/31 faithful, 30/31 relevant, 18/31 citation-correct, 46/49 conditionally complete (`0.9387755102040817`), 16 unsupported sentences. Mean/p95 generation latency was `1444.12`/`2663.41` ms; judge latency was `8176.69`/`11347.33` ms.
- The accepted zero-tolerance faithfulness/citation thresholds failed. Inspection shows both grader false positives on plainly supported real answers and genuine generator errors, including incorrect free-shipping policy and incomplete evidence selection for multi-policy answers. No failing baseline was pinned. Compact evidence: `verification/evidence/phase-3-generation-evaluation-blocker.json`.
- Stopped after the required third failed attempt and marked `p3-generation-evaluation` blocked. Human review must decide whether to expand/rebalance grader calibration and rubric validation, approve a different semantic judge, or change the generator/model. Phase 3 remains in progress and unverified; Phase 4 was not started.

## Follow-up — 2026-09-18: atomic judge calibration

- Resumed only `p3-generation-evaluation` after human approval to repair the judge. Kept the 7B judge, generator, retrieval architecture and generation-quality thresholds unchanged.
- Replaced the overloaded consolidated rubric with atomic sentence-support/citation and answer-point availability/coverage judgments. Added the fixed `generation-judge-calibration-v1` fixture: 62 representative examples across all 31 answerable cases, split into 42 tuning and 20 holdout examples. Added unsupported precision `0.95`, recall `0.90`, and zero-false-unsupported calibration gates while preserving agreement `0.90` and kappa `0.80`.
- Non-model checks passed before calibration: generation 7/7, regression 1/1, system 1/1, harness 54/54, harness structure check, Python compilation and `git diff --check`.
- Calibration attempt 1 exposed mixed-example point indices because batch inputs lacked explicit indices. Attempt 2 added explicit example/sentence/point indices but still mixed indices across differently shaped examples. Attempt 3 grouped homogeneous batch shapes and schema-bounded every index; it completed all 62 examples in 19 judge calls.
- Final calibration still failed: overall agreement `0.8830897703549061`, kappa `0.4690629824630858`, unsupported precision `0.8333333333333334`, recall `0.4838709677419355`, false unsupported `3`; holdout agreement `0.8866666666666667`, kappa `0.5201354911554386`, precision `1.0`, recall `0.7`, false unsupported `0`. Compact evidence: `verification/evidence/phase-3-atomic-judge-calibration-blocker.json`.
- Stopped after the third attempt without running or pinning a baseline. A stronger or specialized semantic judge and a likely runtime-budget change require human approval. Phase 3 remains in progress/unverified; Phase 4 was not started.

## Follow-up — 2026-09-18: judge retry and 2400-second budget decision

- Human review selected local Ollama `qwen2.5:7b-instruct-q4_K_M` and approved raising the generation, regression and system gate budgets from 1800 to 2400 seconds. This exact tag was already used in the latest failed calibration, so the decision is recorded as a bounded retry rather than a new model artifact.
- Added ADR `docs/decisions/0005-phase-3-judge-retry-runtime.md`; updated `docs/EVALUATION.md`, `docs/DEVELOPMENT.md`, the canonical task record and session handoff. All calibration and generation-quality thresholds remain unchanged, and the failed evidence remains historical evidence.
- Cleared the human-decision blocker and resumed only `p3-generation-evaluation` as `in_progress`. After updating the harness documents first, changed the executable generation/regression/system timeout constants and their regression assertions from 1800 to 2400 seconds. No calibration or baseline was run and Phase 4 was not started.
- Verification passed: `.venv/bin/python scripts/harness.py check`, the 7 targeted timeout/recovery tests, `git diff --check`, and post-change `./init.sh` with harness evidence `verification/evidence/15375ffd525b4016bfe923b4ff680662-harness.json`.
- Aligned the evaluator source identity with ADR 0005 so fresh reports hash the current judge/runtime decision as well as ADR 0004 and the calibration fixture. Confirmed the executable judge constant remains `qwen2.5:7b-instruct-q4_K_M`, the three gate budgets are 2400 seconds, and the local Ollama tag resolves to artifact `845dbda0ea48` (4.7 GB). Direct non-model verification passed: generation 7/7, system 2/2, timeout/recovery 7/7, harness structure check and `git diff --check`.

## Follow-up — 2026-09-18: ADR 0005 bounded calibration retry

- Resumed only `p3-generation-evaluation`. Startup `./init.sh` passed with `verification/evidence/9cad4b4f5790475e8621ae7af384d046-harness.json`; Phase 2 and `p3-citations` remained verified. Preserved all existing task changes and did not start Phase 4.
- Re-ran the non-model checks before calibration: generation 7/7, regression 1/1, system 2/2, harness 54/54, harness structure check, Python compilation and `git diff --check` all passed.
- The real local judge resolved to Ollama 0.34.1 artifact `845dbda0ea48ed749caafd9e6037047aa19acfcfd82e704d7ca97d631a0b697e` (Q4_K_M, 4,683,087,332 bytes). The selected generator `qwen2.5:1.5b-instruct` was absent from `ollama list`, but calibration required only the judge.
- The single fresh 62-example, 19-call retry reproduced the prior failure exactly. Overall: agreement `0.8830897703549061`, kappa `0.4690629824630858`, unsupported precision `0.8333333333333334`, recall `0.4838709677419355`, and 3 false unsupported labels. Holdout: agreement `0.8866666666666667`, kappa `0.5201354911554386`, precision `1.0`, recall `0.7`, and 0 false unsupported labels. Evidence: `verification/evidence/phase-3-atomic-judge-calibration-retry-blocker.json`.
- Per ADR 0005, stopped before the generation baseline and did not run generation, regression, system, `verify-task`, or Phase 3 closure gates. No threshold, prompt, fixture, batching, model tag or acceptance rule changed.
- Marked `p3-generation-evaluation` blocked. Human review must choose a materially different judge/grading approach or otherwise revise the accepted decision; the generator model must also be restored before any eventual full baseline. Phase 3 remains in progress and unverified. No downstream task is eligible.
- Post-update `./init.sh` passed with `verification/evidence/fae2c80dc96041a683f8c54338bd963d-harness.json`; harness structure/state, blocker JSON parsing and `git diff --check` also passed.

## Follow-up — 2026-09-18: single-model Phase 3 architecture

- Audited active repository code, task criteria, tests, ADRs and architecture/evaluation/development documentation for LLM model selections. Generation and semantic grading now both use the installed local Ollama tag `qwen2.5:7b-instruct-q4_K_M`; `src/generation.py` no longer selects a separate smaller generator.
- Updated ADR 0003, `ARCHITECTURE.md`, `docs/EVALUATION.md`, `docs/DEVELOPMENT.md`, `feature_list.json`, the handoff and model-contract assertions. Historical progress and verification evidence retain the model identities used by those past runs and do not control current architecture.
- Reopened and freshly verified `p3-generation` and `p3-citations` because the runtime model changed. `verify-task p3-generation` passed with harness `9eca8ba6799248d6b8afc210edf7f3ef`, unit `fe656ed1da65480195028ccdff3f8642`, integration `66f174c8cdaa418b8449033b792292ab` and isolation `67ff43ce48f94cf189155b286294cc07` evidence. `verify-task p3-citations` passed with harness `7243c56779714154a4e9832409b6129f`, unit `f6430657ed754444903433b731fae5c5`, integration `6e60b641cbdd411d9e78a0bd57bd5015` and isolation `62dcb82c67924d2594693e97ff630d29` evidence.
- The `p3-generation-evaluation` task remains blocked only by the failed ADR 0005 judge calibration; the former missing-generator prerequisite is removed. No new calibration or baseline was run, no quality threshold changed, and Phase 4 was not started.
- Final `./init.sh` passed with harness evidence `verification/evidence/084a8b673d8f443d96ff9ae0eb5268cd-harness.json`; the active-tree scan found no model tag other than `qwen2.5:7b-instruct-q4_K_M` outside historical progress and evidence records.
